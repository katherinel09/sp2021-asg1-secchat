...
// Step 3: send Alice's public key to Bob's workstation:
...
// Step 7: Decrypt the file with the private key (on user 2 end)
// $ openssl rsautl -decrypt -inkey bob_private.pem -in message_to_send.enc > message_to_send.txt

int main_decrypt(int key_length, int encrypted_length, unsigned char encrypted)
{
    ...
    decrypted_length = public_decrypt(encrypted, encrypted_length, public_key, decrypted);
    if (decrypted_length == -1)
    {
        printLastError("Public Decrypt didn't work out");
        exit(0);
    }
}
