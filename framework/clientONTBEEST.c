...
{
	log("static int client_process_command(struct client_state *state)");
	assert(state);
	log("assert(state);");
...
static int handle_incoming(struct client_state *state) {
	...
	log("r = select(fdmax+1, &readfds, NULL, NULL, NULL);");
	if (r < 0) {
		log("if (r < 0) {");
		if (errno == EINTR) { log("if (errno == EINTR)"); return 0; }
	...
	if (FD_ISSET(STDIN_FILENO, &readfds)) {
		log("if (FD_ISSET(STDIN_FILENO, &readfds)) {");
   ...
   */
	if (FD_ISSET(state->api.fd, &readfds)) {
		log("if (FD_ISSET(state->api.fd, &readfds)) {");
...
int main(int argc, char **argv) {
	...
	log("struct client_state state;");

	/* check arguments */
	if (argc != 3) usage();
	if (parse_port(argv[2], &port) != 0) usage();
	
	log("if (parse_port(argv[2], &port) != 0) usage();");

	/* preparations */
	client_state_init(&state);
	
	log("client_state_init(&state);");

	/* connect to server */
	fd = client_connect(&state, argv[1], port);
	log("fd = client_connect(&state, argv[1], port);");
	if (fd < 0) return 1;
	log("if (fd < 0) return 1;");

	/* initialize API */
	api_state_init(&state.api, fd);
	log("api_state_init(&state.api, fd);");
